...
const ProductList = ({ currentUser, cart, onAddToCart, onViewDetails }) => {
    ...
    const handleAddToCart = async (productId) => {
        console.log('ProductList - handleAddToCart called with:', { productId, currentUser });
        
        if (!currentUser) {
            console.log('ProductList - No current user, showing error');
            toast.error('برای افزودن به سبد خرید ابتدا وارد شوید!');
            return;
        }

        console.log('ProductList - User is logged in, proceeding with cart addition');

        try {
            const response = await cartAPI.addToCart(productId);
            console.log('ProductList - Cart API response:', response);
            onAddToCart(productId);
            toast.success('محصول به سبد خرید اضافه شد!');
        } catch (error) {
            console.error('ProductList - Error adding to cart:', error);
            console.error('ProductList - Error response:', error.response);
